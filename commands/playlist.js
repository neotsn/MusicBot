  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (stp === 'create') {
        ...
        if (!name) {
          return interaction.reply({ content: lang.msg91, ephemeral: true })
            .catch(e => { })
        }
        ...
        if (playlist?.length > 0) {
          for (let i = 0; i < playlist.length; i++) {
            if (playlist[i]?.playlist?.filter(p => p.name === name)?.length > 0) {
              return interaction.reply({ content: lang.msg92, ephemeral: true })
                .catch(e => { })
            }
          }
        }
        ...
        if (userplaylist?.playlist?.length >= client.config.playlistSettings.maxPlaylist) {
          return interaction.reply({ content: lang.msg93, ephemeral: true })
            .catch(e => { })
        }

        await interaction.reply({ content: `<@${interaction.member.id}>, ${lang.msg94}` })
          .catch(e => { })
        ...
        await interaction.editReply({ content: `<@${interaction.member.id}>, ${lang.msg95}` })
          .catch(e => { })
      }
      ...
      if (stp === 'delete') {
        ...
        const playlist = await db.playlist.findOne({ userID: interaction.user.id })
          .catch(e => { })
        if (!playlist?.playlist?.filter(p => p.name === name).length > 0) {
          return interaction.reply({ content: lang.msg96, ephemeral: true })
            .catch(e => { })
        }
        ...
        await interaction.reply({ content: `<@${interaction.member.id}>, ${lang.msg97}` })
          .catch(e => { })
        ...
        await interaction.editReply({ content: `<@${interaction.member.id}>, ${lang.msg98}` })
          .catch(e => { })
      }
      ...
      if (stp === 'add-music') {
        let name = interaction.options.getString('name')
        if (!name) {
          return interaction.reply({ content: lang.msg99, ephemeral: true })
            .catch(e => { })
        }
        let playlist_name = interaction.options.getString('playlist-name')
        if (!playlist_name) {
          return interaction.reply({ content: lang.msg100, ephemeral: true })
            .catch(e => { })
        }
        ...
        if (playlist?.musics?.filter(m => m.playlist_name === playlist_name).length > max_music) {
          return interaction.reply({ content: lang.msg101.replace('{max_music}', max_music), ephemeral: true })
            .catch(e => { })
        }
        let res
        try {
        ...
        await interaction.reply({ content: `<@${interaction.member.id}>, ${lang.msg102}` })
          .catch(e => { })

        const music_filter = playlist?.musics?.filter(m => m.playlist_name === playlist_name && m.music_name === res[0]?.name)
        if (music_filter?.length > 0) {
          return interaction.editReply({ content: lang.msg104, ephemeral: true })
            .catch(e => { })
        }
        ...
        await interaction.editReply({ content: `<@${interaction.member.id}>, \`${res[0]?.name}\` ${lang.msg105}` })
          .catch(e => { })
      ...
      if (stp === 'delete-music') {
        ...
        let playlist_name = interaction.options.getString('playlist-name')
        if (!playlist_name) {
          return interaction.reply({ content: lang.msg106, ephemeral: true })
            .catch(e => { })
        }
        ...
        if (!music_filter?.length > 0) {
          return interaction.reply({ content: lang.msg54, ephemeral: true })
            .catch(e => { })
        }

        await interaction.reply({ content: `<@${interaction.member.id}>, ${lang.msg108}` })
          .catch(e => { })
        ...
        await interaction.editReply({ content: `<@${interaction.member.id}>, ${lang.msg109}` })
          .catch(e => { })
      }
      ...
      if (stp === 'list') {
        let name = interaction.options.getString('name')
        if (!name) {
          return interaction.reply({ content: lang.msg110, ephemeral: true })
            .catch(e => { })
        }
          ...
          if (playlist[i]?.playlist?.filter(p => p.name === name)?.length > 0) {
            ...
            if (playlist_owner_filter !== interaction.member.id) {
              if (playlist_public_filter === false) {
                return interaction.reply({ content: lang.msg53, ephemeral: true })
                  .catch(e => { })
              }
            }
            ...
            if (!trackl?.length > 0) {
              return interaction.reply({ content: lang.msg111, ephemeral: true })
                .catch(e => { })
            }
            ...
            if (arr === playlist.length) {
              return interaction.reply({ content: lang.msg58, ephemeral: true })
                .catch(e => { })
            }
          }
        }
          ...
          .then(async Message => {
            ...
            collector.on('collect', async (button) => {
              if (button.customId === 'close') {
                collector.stop()
                return button.reply({ content: `${lang.msg68}`, ephemeral: true })
                  .catch(e => { })
              } else {
      ...
      if (stp === 'lists') {
        ...
        if (!playlist?.playlist?.length > 0) {
          return interaction.reply({ content: lang.msg117, ephemeral: true })
            .catch(e => { })
        }
      ...
      if (stp === 'top') {
        ...
        if (!playlists?.length > 0) {
          return interaction.reply({ content: lang.msg114, ephemeral: true })
            .catch(e => { })
        }

        let trackl = []
        playlists.map(async data => {
          data.playlist.filter(d => d.public === true)
            .map(async d => {
