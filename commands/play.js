  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (stp === 'playlist') {
        ...
        let playlist = await db?.playlist?.find()
          .catch(e => { })
        if (!playlist?.length > 0) {
          return interaction.reply({ content: lang.msg52, ephemeral: true })
            .catch(e => { })
        }
        ...
        for (let i = 0; i < playlist.length; i++) {
          if (playlist[i]?.playlist?.filter(p => p.name === playlistw)?.length > 0) {
            ...
            interaction.reply({ content: lang.msg56 })
              .catch(e => { })
            ...
            setTimeout(async () => {
              ...
              await interaction.editReply({
                content: lang.msg57.replace('{interaction.member.id}', interaction.member.id)
              ...
              playlist[i]?.playlist?.filter(p => p.name === playlistw)
                .map(async p => {
      ...
      if (stp === 'normal') {
        ...
        if (!name) {
          return interaction.reply({ content: lang.msg59, ephemeral: true })
            .catch(e => { })
        }

        await interaction.reply({ content: lang.msg61 })
          .catch(e => { })
        try {
