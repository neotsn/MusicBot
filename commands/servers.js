  ...
  run: async (client, interaction) => {
    ...
    if (server) {
      ...
      if (config.shardManager.shardStatus == true) {
        ...
        if (!guild) {
          return interaction.reply({ content: 'I\'m not in that server.', ephemeral: true })
            .catch(e => { })
        }
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(guilds.slice(page * 10, page * 10 + 10)
      ...
      collector.on('collect', async (i) => {
        if (i.customId === 'prev') {
          page--
          embed.setDescription(guilds.slice(page * 10, page * 10 + 10)
            .map(g => `> **${g.name}** \`(${g.id})\` - \`${g.memberCount}\` members`)
            .join('\n'))
