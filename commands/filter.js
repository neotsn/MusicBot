      ...
      interaction.reply({ embeds: [embed], components: [buttons, buttons2, buttons3] })
        .then(async Message => {
          ...
          col.on('collect', async (button) => {
            ...
            if (!filtre) {
              return interaction?.editReply({ content: lang.msg29, ephemeral: true })
                .catch(e => { })
            }
            ...
            if (filters?.includes(filtre?.toLowerCase())) {
              if (queue?.filters?.has(filtre)) {
                ...
                embed?.setDescription(lang.msg31.replace('{filter}', filtre)
                  .replace('{status}', '❌'))
              ...
              embed?.setDescription(lang.msg30.replace('{filters}', filters?.map(mr => `\`${mr}\``)
                .join(', ')))
              if (!filter) {
                return interaction?.editReply({ embeds: [embed] })
                  .catch(e => { })
