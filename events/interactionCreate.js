                  ...
                  if (data?.channels?.length > 0) {
                    ...
                    if (channel_control?.length > 0) {
                      ...
                      if (!channel_filter?.length > 0 && !interaction?.member?.permissions?.has('0x0000000000000020')) {
                        channel_filter = data?.channels?.map(x => `<#${x.channel}>`)
                          .join(', ')
                        return interaction?.reply({ content: lang.msg126.replace('{channel_filter}', channel_filter), ephemeral: true })
                          .catch(e => { })
                      }
                    }
                  }
                  if (interaction?.member?.permissions?.has(props?.permissions || '0x0000000000000800')) {
                          ...
                          if (roleDJ) {
                            if (!interaction?.member?.roles?.cache?.has(roleDJ?.id)) {
                              ...
                              const embed = new EmbedBuilder()
                                ...
                                .setDescription(lang.embed1.replace('{djRole}', roleDJ?.id)
                                  .replace('{cmdMAP}', client.config.opt.DJ.commands.map(astra => '`' + astra + '`')
                                    .join(', ')))
                                .setTimestamp()
                    ...
                    if (props && props.voiceChannel) {
                      if (!interaction?.member?.voice?.channelId) {
                        return interaction?.reply({ content: `${lang.message1}`, ephemeral: true })
                          .catch(e => { })
                      }
                      ...
                      if (guild_me?.voice?.channelId) {
                        if (guild_me?.voice?.channelId !== interaction?.member?.voice?.channelId) {
                          return interaction?.reply({ content: `${lang.message2}`, ephemeral: true })
                            .catch(e => { })
                        }
                      }
                    }
                    ...
                    return interaction?.reply({
                      content: `${lang.message3}: **${props?.permissions?.replace('0x0000000000000020', 'MANAGE GUILD')
                                                        ?.replace('0x0000000000000800', 'SEND MESSAGES') || 'SEND MESSAGES'}**`, ephemeral: true,
                    })
                  }
                } catch (e) {
      ...
      if (config.voteManager.status === true && config.voteManager.api_key) {
        if (config.voteManager.vote_commands.includes(interaction?.commandName)) {
          try {
            ...
            await topApi?.hasVoted(interaction?.user?.id)
              .then(async voted => {
                if (!voted) {
                  const embed2 = new EmbedBuilder()
                    .setTitle('Vote ' + client?.user?.username)
                    .setColor(client?.config?.embedColor)
                    .setDescription(`${config.voteManager.vote_commands.map(cs => `\`${cs}\``)
                      .join(', ')} - ${lang.msg131}
        ...
        switch (interaction?.customId) {
          case 'saveTrack': {
            if (!queue || !queue?.playing) {
              return interaction?.reply({ content: `${lang.msg5}`, embeds: [], components: [], ephemeral: true })
                .catch(e => { })
            } else {
          ...
          case 'time': {
            if (!queue || !queue?.playing) {
              ...
              interaction?.message?.edit({ embeds: [embed] })
                .catch(e => { })
              interaction?.reply({ content: `${lang.msg9}`, embeds: [], components: [], ephemeral: true })
                .catch(e => { })
            }
          }
        }
        ...
        switch (interaction?.customId) {
          case 'playlistModal': {
            ...
            if (!playlist?.playlist?.filter(p => p.name === name).length > 0) {
              return interaction?.reply({ content: `${lang.msg10}`, ephemeral: true })
                .catch(e => { })
            }
            ...
            if (!music_filter?.length > 0) {
              await db?.playlist?.updateOne({ userID: interaction?.user?.id }, {
                ...
                .catch(e => { })
              return interaction?.reply({ content: `<@${interaction?.user?.id}>, **${queue?.songs[0]?.name}** ${lang.msg12}`, ephemeral: true })
                .catch(e => { })
            } else {
              return interaction?.reply({ content: `<@${interaction?.user?.id}>, **${queue?.songs[0]?.name}** ${lang.msg104}`, ephemeral: true })
                .catch(e => { })
            }
          }
            break
